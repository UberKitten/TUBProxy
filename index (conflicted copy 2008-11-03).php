<?php
	class proxied {
		...
		function set_options () {
			...
			if (ereg('moo=', $_SERVER['HTTP_REFERER'])) {
				preg_match('#moo=([^&]+)#', $_SERVER['HTTP_REFERER'], $referer);
					...
					switch ($tag_name) {
						case 'form':
							if (strtolower($attributes['method']) == 'get' || !$attributes['method']) {
								...
								$extra_html = "<input type=\"hidden\" name=\"moo\" value=\"$url\" /><input type=\"hidden\" name=\"__proxy_action\" value=\"redirect_get\" />";
								$attributes['action'] = './';
						...
						case 'head':
							if ($_GET['__proxy_form'] != '0') {
								...
								$extra_html .= 'var moo = '.$this->url_first.';';
								...
								$extra_html .= '</script> <script language="javascript" type="text/javascript" src="./main.js"></script>';
							}
						break;
					}
					if ($attributes) {
		...
		function encode_url ($string, $raw = false, $plain = false) {
			...
			return $raw ? $string : (!$plain ? './?moo=' : '') . base64_encode($string);
		}
		
		function handle_actions () {
			if ($_POST['__proxy_action'] == 'redirect_get') {
				$url = base64_decode($_POST['moo']);
				unset($_POST['__proxy_action'], $_POST['moo']);
				$get = '';
				foreach ($_POST as $key => $value) {
				...
				header('Location: ./?moo=' . base64_encode($_POST['moo']));
				exit;
			}
		}
	...
	function isSearchEngine() {
		if (isset($_GET['testlawl'])) return true;
		if ($isse != 'potato') return $isse;
	...
	$url = 'index.php';
	$url = @parse_url($_GET['moo']) && strlen($_GET['moo']) ? base64_decode($_GET['moo']) : false;
		
	if (!isSearchEngine()) {
		$donttouch = array('avi','bmp','css','doc','exe','fla','gif','ico','jar','java','jpg','js','mov','mp3','png','ppt','pub','rtf','swf','tif','txt','wav','xls','xml','zip');
		...
		if (!strpos($url, '?')) {
    		...
    		$ads = '<div align=center><script type="text/javascript"><!--
google_ad_client = "pub-2743249987059957";      google_ad_width = 468; google_ad_height = 60; google_ad_format = "468x60_as"; google_ad_type = "text_image"; google_color_border = "2d2d2d"; google_color_bg = "2d2d2d"; google_color_link = "ffffff"; google_color_text = "FFFFFF"; google_color_url = "20ACE1";
//-->
</script><script type="text/javascript" src="http://pagead2.googlesyndication.com/pagead/show_ads.js"></script><br></div>';
	}
	
$hits = @file_get_contents('/ramdisk/hits.txt');
if ($hits == null || $hits == '') $hits = 1;
$handle = fopen('/ramdisk/hits.txt', "w");
fwrite($handle,$hits + 1 . "\r\n");
		 
if (isSearchEngine()) {
	include 'home.inc.htm'; 
} else {           
	if (substr($url,0,16) == 'http://gmail.com' || substr($url,0,19) == 'http://www.gmail.com' || substr($url,0,23) == 'http://mail.google.com') {
		die ('<div align=center>Sorry, but GMail is too complex to work through a proxy well.<br>However, feel free to use our <a href="gmail/">GMail-lite</a>, which works great.</div>');
    } elseif (!$url && !$_POST['__proxy_action'] && !isSearchEngine() || substr($url,0,14) == 'http://hal9000' || substr($url,0,18) == 'http://192.168.1.1') {
		$ip = @file_get_contents('http://ub3rk1tten.com/ping.php?pass=closets');
		ob_start();
		include 'home.inc.htm';
		$out = '<script language="javascript" type="text/javascript" src="./base64.js"></script><script language="javascript" type="text/javascript">document.write(decodeBase64(\''.base64_encode(ob_get_contents()).'\'));</script><noscript><div align=center>You need Javascript enabled to use this site!</div></noscript>';                                            
	        ob_end_clean();                   
	        echo $out;  
    } elseif (!in_array($ext,$donttouch) && substr($url,0,31)!='http://images.google.com/imgres' && !isSearchEngine()) {     
        ob_start();
        $proxied = new proxied($url);  
        $out = $ads.'<script language="javascript" type="text/javascript" src="./base64.js"></script><script language="javascript" type="text/javascript">document.write(decodeBase64(\''.base64_encode(ob_get_contents()).'\'));</script><noscript><div align=center>You need Javascript enabled to use this site!</div></noscript>'.$ads;                                            
        ob_end_clean();                   
        echo $out;              
    } else {       
        $proxied = new proxied($url);         
...
/*
$msg = $_SERVER['REMOTE_ADDR'] . ' | ' . date('M j G:i:s Y')  . ' | ' . $url;
